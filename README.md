...
Create a `.env` file in `server/` based on `server/.env.example`, then:
...
Create a `.env` file in `client/` based on `client/.env.example`, then:
...
- Dark mode support
- Community page to browse published images
- Responsive design
...
See [CONTRIBUTING.md](./QuickGPT/CONTRIBUTING.md) for guidelines.

## Changelog

See [CHANGELOG.md](./QuickGPT/CHANGELOG.md) for version history.

## License

MIT — see [LICENSE](./QuickGPT/LICENSE) for details.
